...
namespace Engine
{
    public class Vendor : INotifyPropertyChanged
    {
        ...
        {
            Name = name;
            Inventory = new BindingList<InventoryItem>(); // Start null - no items
        }
        ...
        public void AddItemToInventory(Item itemToAdd, int quantity = 1)
        {
            ...
            {
                // They already have the item, so just increase the quantity
        ...
        public void RemoveItemFromInventory(Item itemToRemove, int quantity = 1)
        {
            ...
            if(item == null)
            {
                // The item is not in player's inventory, so ignore it
